...
def run_model_in_docker(encrypted_data_hash, transaction_id):
    ...
    async_ping_to_frontend(STATUS_MSG["starting_docker"], transaction_id)
...
def recv_decryption_key(mt_id, transaction_id):
    ...
    async_ping_to_frontend(STATUS_MSG["key_got_do"], transaction_id)
    
    return "cova_secret_key_is_the_fanciest"


def send_back_model_params(final_params):
    return make_post_request(final_params, endpoint="/status/final-status")
...
def full_computation_process(encrypted_data_hash, transaction_id, smart_contract_id):
    ...
    final_params = {"transaction_id": transaction_id, "s3_url": "",
        "success": success, "status_code": 1 if success else 3,
        "status_msg": msg}

    if success:
        # upload file to s3
        async_ping_to_frontend(STATUS_MSG["upload_s3"], transaction_id)
...
if __name__ == '__main__':
    # start_docker(0)
    for i in range(5):
        print async_ping_to_frontend('Test Message ' + str(i), 1, status_code=1)
